...
runs of ``goostat``, plus 300x299/2 runs of ``goodiff``). At 30 seconds
...
has been called "`The Mother of All
Demos <http://www.youtube.com/watch?v=a11JDLBXtPQ>`__\ ".
...
to distinguish it from the **graphical user interface**, or GUI, that
most people now use. The heart of a CLI is a **read-evaluate-print
loop**, or REPL: when the user types a command and then presses the
...
shell**. What the user types goes into the shell; it figures out what
commands to run and orders the computer to execute them. Note, the
reason why the shell is called *the shell*: it encloses the operating
