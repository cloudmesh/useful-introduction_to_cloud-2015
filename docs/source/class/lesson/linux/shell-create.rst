...
``/users/nelle``, and use ``ls -F`` to see what it contains::
...
command. Change the working directory to ``thesis`` using ``cd``, then
...
Let's change our working directory to ``thesis`` using ``cd``, then
...
.. tip::
   ...
   A text editor is a program that edits plain text.
   Nano is one example of such a program, there are many more.

   Editors will be covered in a later lesson: :doc:`editors`
...
``^A`` to mean "control-A".) ``nano`` doesn't leave any output on the
screen after it exits, but ``ls`` now shows that we have created a
...
.. caution::
   Deleting Is Forever
    ...
    The Unix shell doesn't have a trash bin that we can recover
...
If we try to remove the entire ``thesis`` directory using ``rm
thesis``, we get an error message::
...
if you are a bad typist. To really get rid of ``thesis`` we must first
...
.. caution::
   With Great Power Comes Great Responsibility
    ...
    This removes everything in the directory, then the directory
    itself.  If the directory contains sub-directories, ``rm -r`` does
    the same thing to them, and so on. It's very handy, but can do a
    lot of damage if used without care.

Moving Files and Directories
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Let's create that directory and file one more time. (Note that this
time we're running ``nano`` with the path ``thesis/draft.txt``, rather
than going into the ``thesis`` directory and running ``nano`` on
``draft.txt`` there.)::
...
``draft.txt`` isn't a particularly informative name, so let's change
the file's name using ``mv``, which is short for "move"::
...
to ``thesis/quotes.txt``, which has the same effect as renaming the
file.  Sure enough, ``ls`` shows us that ``thesis`` now contains one
file called ``quotes.txt``::

    $ ls thesis
    quotes.txt

Just for the sake of inconsistency, ``mv`` also works on directories
--- there is no separate ``mvdir`` command.

Let's move ``quotes.txt`` into the current working directory. We use
``mv`` once again, but this time we'll just use the name of a
directory as the second parameter to tell ``mv`` that we want to keep
...
current working directory. ``ls`` now shows us that ``thesis`` is
empty::
...
The ``cp`` command works very much like ``mv``, except it copies a
...
the current directory and then run that same ``ls`` again.::
...
directory, but it does find the copy in ``thesis`` that we didn't
delete.
...
Suppose that you created a ``.txt`` file in your current directory
...
What command(s) could you run so that the commands below will
...
alphabetical order at each level. The command ``ls -t`` lists things
