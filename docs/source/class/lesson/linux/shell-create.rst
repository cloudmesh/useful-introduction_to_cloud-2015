...
.. tip::
   Touching Files
    ...
    ``touch`` command. Change the working directory to ``thesis``
    using ``cd``, then touch an empty file called ``draft.txt``::
...
.. tip::
   ...
   Listing Recursively and By Time
    ...
    level. The command ``ls -t`` lists things by time of last change,
    with most recently changed files or directories first. In what
    order does ``ls -R -t`` display things?
