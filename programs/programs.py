...
a) write a program that uses loops over both x and y coordinates while x is in 1,2,3,4,5 and y is in 5,4,3,2,1 and prints the x and y coordinate

b) write a program that sums up all values in x and y

c) write a program just like a) but does not print values where x is equal to 2 and y is equal to 4
...
f) read up on classes we will cover this in more detail  next week.
        ...
        in addition the icecream cone will be wrapped into some paper that has an image on it. Images will be Penguin, Apple,
